...
.PHONY: \
	...
	dev-up dev-down dev-load-db dev-load dev-apply dev-restart dev-run dev-update dev-update-apply dev-stripe-secrets dev-auth-keys \
...
AUTH_KEYS_SECRET := $(HELM_RELEASE)-auth-keys
# Key id of the throwaway local signing key generated by dev-auth-keys. It
# must match auth.activeKID in the helm values.
LOCAL_AUTH_KID  := local-dev
...
curl-create-user:
	curl -il -X POST -H 'Content-Type: application/json' -d '{"name":"foo","email":"foo@bar.com","roles":["ADMIN"],"department":"IT","password":"gophers123!AB","passwordConfirm":"gophers123!AB"}' http://localhost:3000/v1/users
...
# Generate a throwaway local signing key for the kind cluster and store it as
# the Secret the auth deployment reads via AUTH_AUTH_KEYS_ENV_VAR. The key is
# local-only and never committed; staging/production keys come from GCP Secret
# Manager (infra/gcp/bootstrap-auth-secret.sh). Existing secrets are kept so
# tokens survive redeploys — delete the secret to force regeneration.
dev-auth-keys:
	@if kubectl get secret $(AUTH_KEYS_SECRET) --namespace=$(NAMESPACE) >/dev/null 2>&1; then \
		echo "Auth keys secret $(AUTH_KEYS_SECRET) already exists; keeping it."; \
	else \
		echo "Generating throwaway local auth signing key (kid: $(LOCAL_AUTH_KID))..."; \
		TMPDIR=$$(mktemp -d); \
		trap 'rm -rf "$$TMPDIR"' EXIT; \
		openssl genpkey -algorithm RSA -out "$$TMPDIR/private.pem" -pkeyopt rsa_keygen_bits:2048 2>/dev/null; \
		jq -n --arg key "$(LOCAL_AUTH_KID)" --rawfile pem "$$TMPDIR/private.pem" '{key: $$key, pem: $$pem}' > "$$TMPDIR/payload.json"; \
		kubectl create secret generic $(AUTH_KEYS_SECRET) --namespace=$(NAMESPACE) \
			--from-file=keys_json="$$TMPDIR/payload.json" --dry-run=client -o yaml | kubectl apply -f - ; \
	fi
