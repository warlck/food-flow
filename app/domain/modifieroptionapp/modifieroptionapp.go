...
// list query is scoped to. A scoping filter (restaurant_id or
// modifier_group_id) is required so list reads can never enumerate another
// organization's catalog; a supplied filter is never treated as authorization
// by itself.
func (a *app) authorizeScope(ctx context.Context, filter modifieroptionbus.QueryFilter) error {
	var restaurantID uuid.UUID

	switch {
	case filter.RestaurantID != nil:
		restaurantID = *filter.RestaurantID
	case filter.ModifierGroupID != nil:
		group, err := a.modifierGroupBus.QueryByID(ctx, *filter.ModifierGroupID)
		if err != nil {
			return errs.New(errs.InvalidArgument, fmt.Errorf("modifier group lookup: %w", err))
		}
		restaurantID = group.RestaurantID
	default:
		return errs.Newf(errs.InvalidArgument, "query requires a restaurant_id or modifier_group_id filter")
	}
