...
- **Name Validation**: The business layer validates names using `^[\p{L}\p{N} '!/\\?|()\[\]{},.:+&_-]{3,100}$`, permitting letters, numbers, spaces, apostrophes (`'`), brackets (`()`, `[]`, `{}`), and specific punctuation/symbols (`!`, `?`, `/`, `\`, `|`, `_`, `-`, `,`, `.`, `:`, `+`, `&`). Other characters (e.g. `"`, angle brackets `<>`) are disallowed. The frontend delegates name validation directly to backend APIs.
...
3. **Validation Gate**: Prove every candidate against the actual code before believing it: open the code, confirm the trigger fires, and check provenance via git history (e.g. `git log -S`) before blaming the branch. Deduplicate mirrored-domain twins (e.g. a menuitem bug and its identical addon twin collapse into one finding — one finding in the ledger, but the fix must cover every twin). Kill anything you cannot demonstrate. Keep a list of the risks you checked and cleared — it becomes the Verified Non-Issues section for free.
...
5. **Coverage Sweeps** (mandatory for spec-based features, recommended otherwise; run before triage):
   - *Spec file-list sweep*: list every file the spec names (e.g., "Affected paths" and per-section file lists) and check each against the branch diff (`git diff --name-only $(git merge-base master HEAD)..HEAD`). A spec-named file with **zero hunks is a mandatory investigation, never a sign of safety**: open it, decide whether the spec requires changes there, and record the verdict (gap vs. legitimately unchanged). Diff-anchored findings passes never open zero-hunk files, so this sweep is the only net that catches unfinished migrations in untouched files.
   - *Sibling sweep*: for every validated finding and every approved fix, check the sibling surfaces for the same defect before it is committed: sibling domains (e.g., the addon app vs. the modifier group/option apps; bus/store twins), sibling pages (desktop vs. mobile), and sibling surfaces (tracking vs. confirmation receipts, admin vs. storefront). A defect in one sibling strongly predicts the same defect in the rest; the "missed sibling" is historically the most common residual gap. Sibling checks that come back clean feed Verified Non-Issues.
   - Record both sweeps and their verdicts in the report's Verification Log.
6. **Human Triage**: Present the validated findings as a compact ledger (one line per finding, grouped by severity) and wait for fix / skip / defer decisions. Never start fix work before triage — declined findings must cost zero engineering time.
7. **Fix Commits**: One atomic conventional commit per approved finding, with its tests riding along, verified green (build + relevant tests) before committing. Reference the finding number in the commit body so the report's Resolved Findings mapping is trivial. Before committing any fix:
   - Re-run the sibling sweep for that defect; if one domain or surface had it, its siblings must be explicitly checked and cleared (or fixed in the same commit).
   - Update any test that asserted the old buggy behavior — a green test asserting the leak is a false pass, not coverage.
   - If the fix removes a legacy behavior, grep for its now-dead plumbing (props, helpers, fallbacks) and remove or migrate it in the same commit.
   - If the fix removes a *compensation* for a legacy behavior (e.g., a dialog picker that substituted for missing cards), verify the underlying behavior it compensated for is migrated in the same change or explicitly deferred — an orphaned compensation turns a legacy quirk into a hard regression.
8. **Final Verification**: Re-run the full gate: build, `go vet`, `gofmt`, affected test suites, and frontend typecheck/lint. The exact commands and outcomes feed the Verification Log.
9. **Render the Report Last**: Generate the report from the validated ledger, never freehand. Split findings into Resolved (finding → fix commit SHA) and Open (accepted, deferred, declined). Compute line anchors once, against the final post-fix tree. `suggestion` blocks are only required for open findings — for resolved ones the fix diff is the suggestion.
...
- **Spec-named zero-hunk files are never assumed fine.** When the spec lists files a feature must touch, an untouched one is a mandatory investigation, not evidence the work is done.
- **Every fix triggers a sibling sweep before it is committed.** If one domain, surface, or page had the defect, its siblings must be explicitly checked and cleared in the same fix cycle.
...
- **Required Report Sections**:
  ...
  7. **Verification Log**: the exact build, vet, format, lint, typecheck, and test commands run, with their outcomes, plus the coverage-sweep outcomes (spec file-list verdicts and sibling-sweep verdicts).
